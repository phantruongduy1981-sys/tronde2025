"""
Trộn Đề Word Online - AIOMT Premium (Single Column Layout)
...
st.set_page_config(
    page_title="AIOMT Premium - Trộn Đề Word",
    page_icon="📄",
    layout="centered", # Chuyển về dạng tập trung ở giữa, không trải rộng
...
<style>
    ...
    .main-header h1 { margin: 0; font-size: 2.5rem; text-shadow: 2px 2px 4px rgba(0,0,0,0.2); }
...
# ==================== CORE LOGIC (GIỮ NGUYÊN LOGIC XỬ LÝ) ====================
...
def process_document_final(file_bytes, num_versions, filename_prefix, auto_fix_img):
    ...
    dom = minidom.parseString(doc_xml) # Base DOM for re-parsing
    ...
    with zipfile.ZipFile(zip_out_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_final:
        for i in range(num_versions):
            ...
            # RE-PARSE logic cho mỗi mã đề để tránh lỗi mất node
            ...
            if parts["p3"]:
                ...
                for q in qs:
                    update_question_label(q[0], g_idx)
                    pb, ans = extract_part3_answer(q[0]), None # Placeholder fix logic
                    # Re-use logic extract
                    val = None
                    for b in q: 
...
def main():
    ...
    <div class="main-header">
        <h1>🎲 AIOMT Premium</h1>
        <p>Hệ thống trộn đề thi trắc nghiệm & Tự luận chuyên nghiệp</p>
    </div>
    """, unsafe_allow_html=True)

    # 1. HƯỚNG DẪN & FILE MẪU
    st.markdown("### 📋 Hướng dẫn & Cấu trúc")
    with st.expander("Xem quy định soạn thảo & Tải file mẫu", expanded=True):
        st.info("""
        **Quy tắc bắt buộc:**
        1. **Câu hỏi:** Bắt đầu bằng `Câu 1`, `Câu 2`...
        2. **Đáp án MCQ:** Phải có đủ A. B. C. D.
        3. **Đáp án đúng:** Phải được **Tô đỏ** (Màu Red/FF0000) hoặc **Gạch chân**.
        4. **Phần tự luận:** Đáp án ghi `ĐS: kết quả` và tô đỏ.
        """)
        
        # Link tải file mẫu
        sample_url = "https://drive.google.com/file/d/1_2zhqxwoMQ-AINMfCqy6QbZyGU4Skg3n/view?usp=sharing"
        st.markdown(f'''
            <a href="{sample_url}" target="_blank" style="text-decoration: none;">
    ...
    # 2. UPLOAD FILE
    st.markdown("### 📂 Tải đề gốc")
    uploaded_file = st.file_uploader("Chọn file Word (.docx) đúng cấu trúc trên", type=["docx"])

    if uploaded_file:
        # ĐỌC FILE 1 LẦN DUY NHẤT ĐỂ TRÁNH LỖI BAD MAGIC NUMBER
        ...
        try:
            ...
            if errors:
                st.markdown('<div class="error-box"><strong>🚫 File chưa đúng cấu trúc:</strong><br>' + '<br>'.join(errors) + '</div>', unsafe_allow_html=True)
                st.stop() # Dừng không hiện nút trộn
                
            if warnings:
                st.warning("⚠️ Cảnh báo: " + "; ".join(warnings))
                
            st.success(f"✅ File hợp lệ! Sẵn sàng trộn.")
            ...
            if st.button("🚀 Bắt đầu trộn đề", type="primary"):
                with st.spinner("Đang xử lý... vui lòng đợi"):
                    try:
                        # TRUYỀN file_bytes ĐÃ ĐỌC VÀO HÀM XỬ LÝ
            ...
            st.error(f"File lỗi: {str(e)}")

    # Footer
    st.markdown("---")
    st.markdown('<div style="text-align:center; color: #888;">© 2025 Phan Trường Duy - THPT Minh Đức</div>', unsafe_allow_html=True)
